...
class AsyncPayoutsResource:
    ...
    async def add_bank_account(
        ...
        *,
        ...
        return _parse_data(
            await self._http.post("/payouts/bank-accounts", body, idempotency_key=idempotency_key),
            PayoutBankAccount,
        )
    ...
    async def request(
        ...
        return _parse_data(
            await self._http.post("/payouts", body, idempotency_key=idempotency_key), Payout
        )

    async def complete_verification(self, *, idempotency_key: str | None = None) -> None:
        """
        ...
        """
        return _data(
            await self._http.post("/payouts/verification", idempotency_key=idempotency_key)
        )
