...
class AsyncGeneratedWebhooksResource:
    def __init__(self, http: AsyncCommetHTTPClient) -> None:
        self._http = http

    async def get(self, id: str) -> Webhook:
        """Retrieve a webhook endpoint by its public ID."""
        return _parse_data(await self._http.get(f"/webhooks/{id}"), Webhook)
    ...
    async def update(
        ...
        *,
        ...
        return _parse_data(
            await self._http.patch(f"/webhooks/{id}", body, idempotency_key=idempotency_key),
            Webhook,
        )

    async def delete(self, id: str) -> DeletedObject:
        """Permanently delete a webhook endpoint."""
        return _parse_data(await self._http.delete(f"/webhooks/{id}"), DeletedObject)

    async def test(self, id: str, *, idempotency_key: str | None = None) -> WebhookTest:
        """Send a test event to a webhook endpoint to verify connectivity."""
        return _parse_data(
            await self._http.post(f"/webhooks/{id}/test", idempotency_key=idempotency_key),
            WebhookTest,
        )
    ...
    async def list(
        ...
        return _parse_data(await self._http.get("/webhooks", query), WebhooksListResult)
    ...
    async def create(
        ...
        *,
        ...
        return _parse_data(
            await self._http.post("/webhooks", body, idempotency_key=idempotency_key),
            CreatedWebhook,
        )
