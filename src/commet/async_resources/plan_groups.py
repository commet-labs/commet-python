...
class AsyncPlanGroupsResource:
    ...
    async def list(
        ...
        *,
        ...
        return await self._http.get("/plan-groups", build_body(limit=limit, cursor=cursor))

    async def get(self, plan_group_id: str) -> ApiResponse[Any]:
        return await self._http.get(f"/plan-groups/{plan_group_id}")
