...
class PromoCodesResource:
    ...
    def get(self, id: str) -> PromoCode:
        """Retrieve a promo code by its public ID."""
        return _parse_data(self._http.get(f"/promo-codes/{id}"), PromoCode)
    ...
    def update(
        ...
        *,
        ...
        return _parse_data(
            self._http.patch(f"/promo-codes/{id}", body, idempotency_key=idempotency_key), PromoCode
        )

    def list(self, *, cursor: str | None = None, limit: int | None = None) -> PromoCodesListResult:
        """List promo codes with cursor-based pagination."""
        query = build_body(cursor=cursor, limit=limit)
        return _parse_data(self._http.get("/promo-codes", query), PromoCodesListResult)
