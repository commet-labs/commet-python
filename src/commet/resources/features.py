...
class FeaturesResource:
    ...
    def get(self, code: str) -> Feature:
        """Get a single feature definition by code from the organization's feature catalog."""
        return _parse_data(self._http.get(f"/features/{code}"), Feature)
    ...
    def update(
        ...
        *,
        ...
        return _parse_data(
            self._http.patch(f"/features/{code}", body, idempotency_key=idempotency_key), Feature
        )

    def delete(self, code: str) -> DeletedObject:
        """Delete a feature. Fails if the feature is attached to active plans or has an active add-on."""
        return _parse_data(self._http.delete(f"/features/{code}"), DeletedObject)

    def list(self) -> FeaturesListResult:
        ...
        return _parse_data(self._http.get("/features"), FeaturesListResult)
    ...
    def create(
        ...
        *,
        ...
        return _parse_data(
            self._http.post("/features", body, idempotency_key=idempotency_key), Feature
        )
