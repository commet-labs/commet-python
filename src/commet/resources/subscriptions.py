...
class SubscriptionsResource:
    ...
    def list(
        ...
        return _parse_list(self._http.get("/subscriptions", query), Subscription)
    ...
    def create(
        ...
        *,
        ...
        return _parse(
            self._http.post("/subscriptions", body, idempotency_key=idempotency_key), Subscription
        )

    def get(self, id: str) -> ApiResponse[Subscription]:
        """Get a subscription by its public ID, regardless of status (including pending_payment and past_due)."""
        return _parse(self._http.get(f"/subscriptions/{id}"), Subscription)

    def get_active(self, *, customer_id: str) -> ApiResponse[Subscription | None]:
        """Get the active subscription for a customer. Returns null if none."""
        query = build_body(customer_id=customer_id)
        return _parse(self._http.get("/subscriptions/active", query), Subscription)
    ...
    def reactivate(
        ...
        return _parse(
            self._http.post(f"/subscriptions/{id}/reactivate", idempotency_key=idempotency_key),
            ReactivatedSubscription,
        )
    ...
    def create_recovery_link(
        ...
        return _parse(
            self._http.post(f"/subscriptions/{id}/recovery-link", idempotency_key=idempotency_key),
            RecoveryLink,
        )
    ...
    def deactivate_addon(self, id: str, addon_id: str) -> ApiResponse[DeletedSubscriptionAddon]:
